package com_powderRoom_BlogTest;
...
import ObjectRepo.BlogsDetailsPage;
...
public class BlogsTest extends BaseClass {
	...
	@Test
	public void TC_Wl_05_validateShopNowFunctionalityTest() throws InterruptedException {
		HomePage homePage = new HomePage(driver);
		Thread.sleep(2000);
		Assert.assertEquals(true, homePage.getHomeTab().isDisplayed());
		homePage.getHomeTab().click();
	...
	@Test
	public void TC_Wl_06_validateWolooPointsPopupTest() throws InterruptedException {
		HomePage homePage = new HomePage(driver);
		Thread.sleep(2000);
		Assert.assertEquals(true, homePage.getHomeTab().isDisplayed());
		homePage.getHomeTab().click();
	...
	@Test
	public void TC_Wl_07_validateWolooPointsPopupUIFunctionalityTest() throws InterruptedException
	{
		HomePage homePage = new HomePage(driver);
		Thread.sleep(2000);
		Assert.assertEquals(true, homePage.getHomeTab().isDisplayed());
	...
	@Test
	public void TC_Wl_08_validateBlogsDetailScreenUITest() throws InterruptedException
	{
		HomePage homePage = new HomePage(driver);
		Thread.sleep(2000);
		Assert.assertEquals(true, homePage.getHomeTab().isDisplayed());
		...
		WolooPointsPopup wolooPopup=new WolooPointsPopup(driver);
		wolooPopup.getGoBackButton().click();
		
		BlogsDetailsPage blogDetPage=new BlogsDetailsPage(driver);
		blogDetPage.verifyBlogsDetailsScreenUI();
		
		try {
			if(blogDetPage.getVideoView().isDisplayed())
			{
				blogDetPage.verifyVideoPlayfunctionality();
			}
		}
		catch (Exception e) {
			e.printStackTrace();
		...
		try {
			if(!blogDetPage.getVideoView().isDisplayed())
			{
				blogDetPage.verifyImagefunctionality();
			}
		}
		catch (Exception e) {
			e.printStackTrace();
